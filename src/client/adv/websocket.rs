...
use crate::client::traits::{WebSocketClientAsyncTrait, WebSocketClientTrait};
use crate::constants::websocket::{PUBLIC_ENDPOINT, SECURE_ENDPOINT};
use crate::errors::CbError;
use crate::jwt::Jwt;
use crate::models::adv::websocket::{
    Channel, Endpoint, EndpointType, Message, SecureSubscription, Subscription,
    UnsignedSubscription, WebSocketSubscriptions,
};
use crate::time;
...
#[async_trait]
impl WebSocketClientTrait<Message, Channel> for WebSocketClient {
    /// Getter method for `enable_public`
    fn public_enabled(&self) -> bool {
        self.enable_public
    }
    /// Getter method for `enable_user`
    fn auth_enabled(&self) -> bool {
        self.enable_user
    }
    /// Getter method for `uris`
    fn uris(&self) -> HashMap<EndpointType, String> {
        self.uris.clone()
    }
    /// Getter method for `max_retries`
    fn max_retries(&self) -> u32 {
        self.max_retries
    }
    /// Getter method for `public_tx.lock()`
    async fn public_tx_lock(&self) -> MutexGuard<Option<SplitSink<Socket, WsMessage>>> {
        self.public_tx.lock().await
    }
    /// Getter method for `secure_tx.lock()`
    async fn auth_tx_lock(&self) -> MutexGuard<Option<SplitSink<Socket, WsMessage>>> {
        self.secure_tx.lock().await
    }
    /// Getter method for `subscriptions.lock()`
    #[allow(private_interfaces)]
    async fn subscriptions_lock(&self) -> MutexGuard<WebSocketSubscriptions> {
        self.subscriptions.lock().await
    }
    /// Getter method for `public_bucket.lock()`
    #[allow(private_interfaces)]
    async fn public_bucket_lock(&self) -> MutexGuard<TokenBucket> {
        self.public_bucket.lock().await
    }
    /// Getter method for `secure_bucket.lock()`
    #[allow(private_interfaces)]
    async fn auth_bucket_lock(&self) -> MutexGuard<TokenBucket> {
        self.secure_bucket.lock().await
    }
    ...
    async fn handle_reconnect(&mut self, endpoint_type: &EndpointType) -> CbResult<Endpoint> {
        ...
        let subs = {
            let subscriptions = self.subscriptions_lock().await;
...
impl WebSocketClientAsyncTrait<Message, Channel> for WebSocketClient {}
